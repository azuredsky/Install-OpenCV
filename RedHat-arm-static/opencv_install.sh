...
mkdir -p build
cd build
cmake -D CMAKE_BUILD_TYPE=RELEASE -D BUILD_SHARED_LIBS=0  -D BUILD_opencv_world=ON -DWITH_OPENCL=0 -DWITH_TBB=1 -DWITH_PNG=1 -DWITH_TIFF=1 -DWITH_JPEG=1 -DWITH_JASPER=1 -D CMAKE_INSTALL_PREFIX=./install -DCMAKE_TOOLCHAIN_FILE=../platforms/linux/aarch64-gnu.toolchain.cmake ..
make -j 6
sudo make install
